...
def full_path():
    ...
    csv_full_path = os.path.join(script_dir, LIST_NAME)
    return db_full_path, csv_full_path


class DailyUpdate:
    def __init__(self):
        self.db_path, self.parquet_path = full_path()
        self.conn = None
        self.cursor = None
        self.table_name = None
        # 在类顶部定义一次，全局复用
        self.STOCK_LIST_SCHEMA = {
            "stock_code": "string",
            "stock_name": "string"
        }

    def get_all_stock_list_old(self):
        try:
            print("正在获取沪深京A股股票列表...")
    ...
    def get_all_stock_list(self):
        try:
            ...
            # 强制定义 dtype，防止类型错乱
            stock_df = stock_df.astype(self.STOCK_LIST_SCHEMA)

            # 保存 Parquet
            stock_df.to_parquet(self.parquet_path, index=False, engine="pyarrow")
    ...
    def run_sync_process(self, data_source="akshare", adjust_type="bfq", debug_mode=False):
        ...
        # ===================== ✅ 正确读取 parquet（无 dtype，不报错） =====================
        try:
            all_stocks = pd.read_parquet("./utils/stock_list.parquet")
        except Exception as e:
            print("❌ 请确保 ./utils/stock_list.parquet 存在")
            print(f"错误信息：{e}")
            return
