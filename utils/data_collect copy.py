...
class StockDataCollector():
    ...
    def __init__(self):
        ...
        with open(config_full_path, "r", encoding="utf-8") as f:
            ...
            debug_mode = config["data_collect_config"]["debug_mode"]

            stock_list_source = config["data_collect_config"]["stock_list_source"]
            stock_history_source = config["data_collect_config"]["stock_history_source"]
            adjust_type_bfq = config["data_collect_config"]["adjust_type_bfq"]
            adjust_type_qfq = config["data_collect_config"]["adjust_type_qfq"]

        self.config = {
            "stock_list_source": stock_list_source,  # 可选：akshare/tushare
            "stock_history_source": stock_history_source,  # 可选：akshare/tushare

            "adjust_type_bfq": adjust_type_bfq,  # 不复权
            "adjust_type_qfq": adjust_type_qfq,  # 前复权
            "tushare_token": tushare_token,  # Tushare token（需自行申请）
    ...
    def get_stock_data(self,
        ...
        if self.config["debug_mode"]:
            print(
                f"🔍 调试信息：AKShare调用参数 → symbol={normalized_code}, start={start_date}, end={end_date}, adjust={self.config['adjust_type']}")
